"""
RSS feed news source — fetches headlines from RSS/Atom feeds via feedparser.
"""
import logging
...
class RSSSource(NewsSource):
    """Fetches news from a single RSS/Atom feed."""

    def __init__(self, feed_name: str, feed_url: str, tier: int = 2):
    ...
    def fetch(self) -> list[RawNewsItem]:
        """Fetch latest entries from the RSS feed."""
        ...
        if feed.get("status") == 304:
            logger.debug("RSS %s: not modified since last fetch", self._name)
            return []
        ...
        for entry in feed.get("entries", []):
            item = _parse_entry(entry, self._name)
            if item:
                items.append(item)
...
def _parse_entry(entry: dict[str, Any], source_name: str) -> Optional[RawNewsItem]:
    """Parse a single feedparser entry into a RawNewsItem."""
    ...
    return RawNewsItem(
        ...
        raw_data={"id": entry.get("id", link)},
    )


def create_rss_sources(cfg: dict[str, Any]) -> list[RSSSource]:
    """Create RSS sources from config."""
